...
WHERE Trans_Init_Date BETWEEN '2016-11-15' AND '2016-11-17';

-- List customers who made transactions using cash 
SELECT c.* FROM customer_information c
JOIN transaction_information t ON c.CustomerID = t.CustomerID
JOIN payment p ON t.TransactionID = p.TransactionID
WHERE p.Mode = 'cash';

-- List suppliers with the quantity of products they supplied 
SELECT s.SName, p.SupplierID, COUNT(p.ProductID) AS ProductCount
FROM supplier_information s
JOIN product p ON s.SupplierID = p.SupplierID
GROUP BY p.SupplierID;

-- List customers who made transactions with a discount 
SELECT DISTINCT c.*
...
SELECT * FROM transaction_information
ORDER BY Trans_Init_Date DESC
LIMIT 1;

-- Retrieve transactions with a specific product and quantity
SELECT * FROM transaction_detail
WHERE ProductID = 1 AND Quantity = 20;


-- List suppliers and the total quantity of products they supplied 
SELECT s.SName, p.SupplierID, SUM(p.Quantity_in_stock) AS TotalSupplied
FROM supplier_information s
JOIN product p ON s.SupplierID = p.SupplierID
GROUP BY p.SupplierID;


-- Find customers who made transactions above a certain amount
SELECT c.*
FROM customer_information c
JOIN transaction_information t ON c.CustomerID = t.CustomerID
JOIN transaction_detail td ON t.TransactionID = td.TransactionID
WHERE td.Total_Amount > 2000;
